...
def place_second_rover(mars):
    if not mars.add_rover((mars.rows-1,mars.cols-1)):
        for x in range(mars.rows-1,-1,-1):
            for y in range(mars.cols-1,-1,-1):
                if mars.grid[x][y]==0:
                    mars.add_rover((x,y)); return

def main():
    mars=MarsGrid(6,6,num_craters=5,num_sites=3)
    mars.add_rover((0,0)); place_second_rover(mars)
    grid=mars.get_grid(); start,goal=mars.rovers[0],list(mars.sites)[0]
    print("Start:",start,"Goal:",goal); print("Sites:",mars.sites); print("Craters:",mars.craters)

    print("\n=== Mars Rover Exploration Menu ===")
    print("1. BFS\n2. DFS\n3. UCS\n4. A*\n5. CSP\n6. Adversarial\n7. Run ALL")
    choice=input("Choose algorithm: ")

    pygame.init(); rows,cols=len(grid),len(grid[0])
    screen=pygame.display.set_mode((cols*60,rows*60)); pygame.display.set_caption("Mars Rover Exploration")

    if choice=="1":
        path=bfs(grid,start,goal); print("BFS Path:",path)
        if path: animate_path(screen,grid,start,path,save_gif=True,gif_name="bfs.gif")
    elif choice=="2":
        path=dfs(grid,start,goal); print("DFS Path:",path)
        if path: animate_path(screen,grid,start,path,save_gif=True,gif_name="dfs.gif")
    elif choice=="3":
        path=ucs(grid,start,goal); print("UCS Path:",path)
        if path: animate_path(screen,grid,start,path,save_gif=True,gif_name="ucs.gif")
    elif choice=="4":
        path=a_star(grid,start,goal); print("A* Path:",path)
        if path: animate_path(screen,grid,start,path,save_gif=True,gif_name="astar.gif")
    elif choice=="5":
        ...
        for name,path in paths.items():
            ...
            if path: animate_path(screen,grid,start,path,save_gif=True,gif_name=f"{name}.gif")
        # Run CSP
        assignment,cost=backtracking_csp(mars.rovers,list(mars.sites))
        print("CSP Assignment:",assignment,"Total Cost:",cost)
        animate_csp(screen,grid,mars.rovers,assignment,bfs,save_gif=True,gif_name="csp.gif")
