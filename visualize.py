...
def animate_path(screen,grid,start,path,save_gif=False,gif_name="path.gif"):
    frames=[]
    for step in path:
        for e in pygame.event.get():
            if e.type==pygame.QUIT: pygame.quit(); sys.exit()
        screen.fill((255,255,255)); draw_grid(screen,grid,[step]); pygame.display.flip()
    ...
    while sites:
        for player in [0,1]:
            ...
            if move:
                ...
                for e in pygame.event.get():
                    if e.type==pygame.QUIT: pygame.quit(); sys.exit()
                screen.fill((255,255,255)); draw_grid(screen,grid,[positions[0],positions[1]])
                pygame.display.flip()
                if save_gif: capture_frame(screen,frames)
                time.sleep(0.5)
            if not sites: break
